...
class ListFamille extends Component {
    ...
    getTotalPages = (page,size) => {
        axios.get('http://localhost:8081/familles/getTotalPages/'+page+'/'+size)
        .then(response => {
            this.setState({
    ...
    getFamilles = (page , size) => {
        axios.get('http://localhost:8081/familles/'+page+'/'+size)
        .then(response => {
            this.setState({
    ...
    delete(familleName) {
        axios.get(`http://localhost:8081/familles/getId/`+familleName)
        .then(response => {
            this.setState({
    ...
    deleteFromDB() {
        axios.delete('http://localhost:8081/familles/delete/'+this.state.FamilleID)
                this.setState({
                    errorAlert : false
                })
            this.handleClickClose();
    ...
    render(){
        return (
            <div>
                <ReferentielNav/>
                <h3 className="head-title">Liste  Familles</h3>
                {this.state.errorAlert ? <Alert severity="error" className='alert'>{this.state.errorAlertContent}</Alert> : <></> }
                <div className='content-zone'>
                    
                    <Link to='/dossiers/referentiel/familles/New' className='add-button'>
                        ...
                        <Table sx={{ minWidth: 900 }} aria-label="simple table">
                            ...
                            {this.state.familles.map(famille => (
                                <TableRow key={famille}>
                                    ...
                                    <TableCell>
                                        <DeleteIcon onClick={()=>this.delete(famille)} 
                    ...
                    >
                        ...
                        <DialogContentText id="alert-dialog-description">
                            Etes-vous sûrs de vouloir supprimer {this.state.familleDel} ?
                        </DialogContentText>
                        </DialogContent>
                        <DialogActions>
