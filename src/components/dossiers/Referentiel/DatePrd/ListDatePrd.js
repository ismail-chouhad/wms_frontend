...
class ListDatePrd extends Component {
    ...
    getTotalPages = (page,size) => {
        axios.get('http://localhost:8081/datePrds/getTotalPages/'+page+'/'+size)
        .then(response => {
            this.setState({
    ...
    getDatePrds = (page , size) => {
        axios.get('http://localhost:8081/datePrds/'+page+'/'+size)
        .then(response => {
            this.setState({
                datePrds: response.data
            })
        })
    }

    componentDidMount() {
        this.getDatePrds(0,5);
        this.getTotalPages(0,5);
    }

    delete(datePrdDate) {
        axios.get(`http://localhost:8081/datePrds/getId/`+datePrdDate)
        .then(response => {
            this.setState({
                DatePrdID : response.data ,
                datePrdDel : datePrdDate
            })                 
        })
        .then(res => {      
    ...
    deleteFromDB() {
        axios.delete('http://localhost:8081/datePrds/delete/'+this.state.DatePrdID)
                this.setState({
                    errorAlert : false
                })
            this.handleClickClose();
            this.getDatePrds(this.state.pageNav,5);
            this.getTotalPages(this.state.pageNav,5);
    }
    setPage(page) {
        this.setState({ pageNav : page })
        this.getDatePrds(page,5);
    ...
    render(){
        return (
            <div>
                <ReferentielNav/>
                <h3 className="head-title">Liste des dates de productions</h3>
                {this.state.errorAlert ? <Alert severity="error" className='alert'>{this.state.errorAlertContent}</Alert> : <></> }
                <div className='content-zone'>
                    
                    <Link to='/dossiers/referentiel/datePrds/New' className='add-button'>
                    ...
                    <TableContainer component={Paper} className='table'>
                        <Table sx={{ minWidth: 900 }} aria-label="simple table">
                            ...
                            <TableBody>
                            {this.state.datePrds.map(datePrd => (
                                <TableRow key={datePrd}>
                                    <TableCell>
                                        {datePrd}
                                    </TableCell>
                                    <TableCell>
                                        <Link to={'/dossiers/referentiel/datePrds/edit/'+datePrd} >
                                            <EditIcon color="primary"/>
                                        </Link>
                                    </TableCell>
                                    <TableCell>
                                        <DeleteIcon onClick={()=>this.delete(datePrd)} 
                    ...
                    >
                        ...
                        <DialogContentText id="alert-dialog-description">
                            Etes-vous sûrs de vouloir supprimer {this.state.datePrdDel} ?
                        </DialogContentText>
                        </DialogContent>
                        <DialogActions>
