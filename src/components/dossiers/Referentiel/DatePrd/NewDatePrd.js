...
export default function NewDatePrd() {

    const buttonStyle = {  marginTop: "10px" }
    const [datePrdDate,setDatePrdDate] = useState('')
    ...
    const handleClick=(e)=>{
        ...
        axios.post('http://localhost:8081/datePrds/save/'+datePrdDate , {
        })
        .then(res => {
            console.log(res);
            console.log(res.data);
            if(res.data === "DatePrd ajoutée")
            {
                setSuccessAlertContent(res.data);
                setSuccessAlert(true);
                setErrorAlert(false);
            }
            else if(res.data === "DatePrd existe déjà")
            {
  ...
  return (
    <div>
        <ReferentielNav/>
        <h3 className="head-title">Ajouter une date de prodution</h3>
