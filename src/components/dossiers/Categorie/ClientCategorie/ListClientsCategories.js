...
class ListClientsCategories extends Component {
    ...
    getTotalPages = (page,size) => {
        axios.get('http://localhost:8081/clientCategories/getTotalPages/'+page+'/'+size)
        .then(response => {
            this.setState({
    ...
    getCategories = (page , size) => {
        axios.get('http://localhost:8081/clientCategories/'+page+'/'+size)
        .then(response => {
            this.setState({
    ...
    deleteCat(categorieName) {
        axios.get(`http://localhost:8081/clientCategories/getId/`+categorieName)
        .then(response => {
            this.setState({
    ...
    deleteCatFromDB() {
        axios.delete('http://localhost:8081/clientCategories/delete/'+this.state.cltCategorieID)
                this.setState({
                    errorAlert : false
                })
            this.handleClickClose();
    ...
    render(){
        return (
            <div>
                <CategorieNav/>
                <h3 className="head-title">Liste Clients Categories</h3>
                {this.state.errorAlert ? <Alert severity="error" className='alert'>{this.state.errorAlertContent}</Alert> : <></> }
                <div className='content-zone'>
                    
                    <Link to='/dossiers/categories/cltCategories/New' className='add-button'>
                        ...
                        <Table sx={{ minWidth: 900 }} aria-label="simple table">
                            ...
                            {this.state.categories.map(categorie => (
                                <TableRow key={categorie}>
                                    ...
                                    <TableCell>
                                        <Link to={'/dossiers/categories/cltCategories/edit/'+categorie} >
...
export default ListClientsCategories;