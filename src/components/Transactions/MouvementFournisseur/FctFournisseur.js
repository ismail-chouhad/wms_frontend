...
export default class FctFournisseur extends Component {
    ...
    getTotalPages = (page,size) => {
        axios.get('http://localhost:8081/factureFournisseur/totalPages/'+page+'/'+size)
        .then(response => {
            this.setState({
    ...
    getFactures = (page , size) => {
        axios.get('http://localhost:8081/factureFournisseur/'+page+'/'+size)
        .then(response => {
            this.setState({
  ...
  render() {
    return (
      <div>
        ...
        <h3 className='head-title'>Liste des factures fournisseur</h3>
                ...
                <Table sx={{ minWidth: 900 }} aria-label="simple table">
                    ...
                    {this.state.factures.map(facture => (
                        <TableRow key={facture[0]}>
                            ...
                            <TableCell>
                                <Link to={'/transactions/mvfournisseurs/facturation/vue/'+facture[0]} >
