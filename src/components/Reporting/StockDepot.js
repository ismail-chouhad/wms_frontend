...
export default class StockDepot extends Component {
    ...
    getTotalPages = (page,size) => {
        axios.get('http://localhost:8081/depots/totalPages/'+page+'/'+size)
        .then(response => {
            this.setState({
    ...
    getDepots = (page , size) => {
        axios.get('http://localhost:8081/depots/stockGlobal/'+page+'/'+size)
        .then(response => {
            this.setState({
  ...
  render() {
    return (
      <div>
        <ReportingNav/>
        <h3 className='head-title'>Etat de stock par dépôts</h3>
